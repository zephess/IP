...
public class DialogueManager : MonoBehaviour
{
    ...
    private Queue<string> queueBuffer = new Queue<string>(); // Queue to hold incoming dialogue messages
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {            
            queueBuffer.Enqueue(txt[textIndex]); // Add the current text to the queue
            textIndex++;
            if (!isRunning)
            {
    ...
    private IEnumerator TypeText()
    {
        ...
        {
            string currentMessage = queueBuffer.Dequeue(); // Get the next message from the queue
            text.text = "";
