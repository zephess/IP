...
public class elevatorScript : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            gameObject.GetComponent<BoxCollider>().enabled = false; // Disable the trigger to prevent multiple activations
            other.transform.SetParent(transform); // Parent the player to the elevator
        ...
        if (other.tag.Equals("ControllerEnable"))
        {
            src.PlayOneShot(elevatorCrash);
        }
    ...
    private IEnumerator MoveGrate(Transform target, float distance, float time)
    {
        ...
        if (!hasElevatorMoved)
        {
            ...
            StartCoroutine(MoveElevator(transform, -distanceToMove, 24f)); // Move the elevator down by 3 units over 3 seconds
        }
    ...
    private IEnumerator MoveElevator(Transform target, float distance, float time)
    {
        ...
        target.position = endPos; // Ensure it ends at the exact position
        src.PlayOneShot(elevatorCrash); // Play the elevator crash sound
